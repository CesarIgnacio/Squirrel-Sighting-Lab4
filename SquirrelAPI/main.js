...
app.get("/locations/:report_id", (req, res) => { //get report location
    const repId = req.params.report_id;
    connection.query("SELECT building FROM sight_location WHERE report_id=?", [repId], (err, result, fields) => {
        if (err) {
            console.log(err);
        } else {
            res.send(result);
...
app.put("/locations/:report_id", (req,res) => {
    ...
    connection.query("UPDATE sight_location SET building=? WHERE report_id=?", [buildingName, repId], (err, result) => {
        if(err) {
            console.log(err)
        } else {
...
app.get("/description/:report_id", (req, res) => { //get report description
    const repId = req.params.report_id;
    connection.query("SELECT total_seen, phys_desc, squirrel_action FROM squirrel_desc WHERE report_id=?", [repId], (err, result, fields) => {
        if (err) {
            console.log(err);
        } else {
            res.send(result);
...
app.put("/description/:report_id", (req, res) => {
    ...
    connection.query("UPDATE squirrel_desc SET total_seen=?, phys_desc=?, squirrel_action=? WHERE report_id=?", [total_seen, phys_desc, squirrel_action, repId], (err, result) => {
        if(err) {
            console.log(err);
        } else {
...
app.get("/timeandplace/:report_id", (req, res) => {
    const repId = req.params.report_id;
    connection.query("SELECT area_id, time_sighted FROM sight_report WHERE report_id=?", repId, (err, result) => {
        if(err) {
            console.log(err);
        } else {
            res.send(result);
...
app.put("/timeandplace/:report_id", (req, res) => {
    ...
    connection.query("UPDATE sight_report SET area_id=?, time_sighted=? WHERE report_id=?", [area_id, time_sighted, repId], (err, result) => {
        if(err) {
            console.log(err);
        } else {
...
app.delete("/clear", (req,res)=>{
    connection.query("DELETE FROM sight_location; DELETE FROM squirrel_desc; DELETE FROM sight_report",(err,result)=>{
        if(err){
            console.log(err);
        } else {
            res.send(result);
...
app.delete("/clear/:report_id", (req,res) => {
    const repId = req.params.report_id;
    connection.query("DELETE FROM sight_location WHERE report_id=?; DELETE FROM squirrel_desc WHERE report_id=?; DELETE FROM sight_report WHERE report_id=?", [repId, repId, repId], (err, result) => {
        if(err) {
            console.log(err);
        } else {
